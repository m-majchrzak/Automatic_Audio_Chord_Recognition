...
def read_Billboard():
    ...
    adir = os.path.join(dir, "McGill-Billboard-MIREX") # directory of annotations
    fdir = os.path.join(dir, "McGill-Billboard-Features") # directory of features
    foldernames = sorted(os.listdir(fdir)) # serial numbers of all pieces

    id_dir = os.path.join(dir, "billboard-2.0-index.csv")
    ...
    for name in foldernames:
        if name in train_set or name in test_set:
            # print(name)
            filedir = os.path.join(adir, name, 'majmin.lab')
            annotation = []
    ...
    for name in foldernames:
        if name in train_set or name in test_set:
            filedir = os.path.join(fdir, name, 'bothchroma.csv')
    ...
    outputdir = os.path.join('preprocessed_data', 'Billboard_data_mirex_Mm.pkl')
...
def augment_Billboard():
    ...
    inputdir = os.path.join('preprocessed_data', 'Billboard_data_mirex_Mm.pkl')
    ...
    for shift in range(12):
        ...
        # save the preprocessed data
        outputdir = os.path.join('preprocessed_data', 'Billboard_data_mirex_Mm_shift_' + str(shift) + '.pkl')
...
def segment_Billboard():
    ...
    for shift in range(12):
        inputdir = os.path.join('preprocessed_data', 'Billboard_data_mirex_Mm_shift_' + str(shift) + '.pkl')
        with open(inputdir, 'rb') as input_data:
        ...
        # save the preprocessed data
        outputdir = os.path.join('preprocessed_data', 'Billboard_data_mirex_Mm_shift_segment_' + str(shift) + '.pkl')
        with open(outputdir, "wb") as output_file:
...
def reshape_Billboard():
    ...
    for shift in range(12):
        inputdir = os.path.join('preprocessed_data', 'Billboard_data_mirex_Mm_shift_segment_' + str(shift) + '.pkl') # with segment
        with open(inputdir, 'rb') as input_data:
            BillboardData_shift = pickle.load(input_data)
        """BillboardData_shift = {'op': {'chroma': array, 'TC': array, 'chord': array, 'chordChange': array}, ...}"""
        ...
        # save the preprocessed data
        outputdir = os.path.join('preprocessed_data', 'Billboard_data_mirex_Mm_reshape_' + str(shift) + '.pkl' ) # with segment
        with open(outputdir, "wb") as output_file:
...
def split_dataset(train_set, test_set):
    ...
    inputdir = os.path.join('preprocessed_data', 'Billboard_data_mirex_Mm_reshape_0.pkl') # with segment
    with open(inputdir, 'rb') as input_data:
        BillboardData_reshape_0 = pickle.load(input_data)
        """BillboardData_reshape = {'op': {'chroma': array, 'TC': array, 'chord': array, 'chordChange': array, 'sequenceLen': array, 'nSequence': array}, ...}"""

    # split dataset into training, validation, and testing sets
    dir = os.path.join('McGill-Billboard', 'McGill-Billboard-Features')
    ...
    for shift in range(12):
        inputdir = os.path.join('preprocessed_data', 'Billboard_data_mirex_Mm_reshape_' + str(shift) + '.pkl') # with segment
        with open(inputdir, 'rb') as input_data:
    ...
    outputdir = os.path.join('preprocessed_data', 'Billboard_data_mirex_Mm_model_input_final.npz') # with segment
    with open(outputdir, "wb") as output_file:
